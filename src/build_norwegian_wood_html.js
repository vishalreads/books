...
const mdPath = path.join(__dirname, '..', 'docs', 'distillations', 'norwegian-wood', 'master-notes.md');
const mdContent = fs.readFileSync(mdPath, 'utf-8');

// Lightweight Markdown to HTML function
...
<head>
  ...
  <title>Master Codex: Norwegian Wood by Haruki Murakami | Intellectualist</title>
  <style>
    :root {
      --bg-canvas: #fbf9f4;
      --bg-card: #ffffff;
...
<div class="container">
  <header class="gazette-header">
    ...
    <div class="meta-bar">
      <span>INTELLECTUALIST MASTER CODEX SERIES • NO. 007</span>
      <span class="badge-tier">Tier 1/2: Elegiac Existential Realism & Forensic Trauma</span>
    </div>
    <h1 class="title">Master Codex: Norwegian Wood</h1>
    <div class="subtitle">Haruki Murakami's Definitive Treatise on Grief, Suicide Contagion, and the Integration of Death into Life</div>
    <div style="font-size: 0.92rem; color: var(--text-muted); margin-bottom: 12px;">
      <strong>Source Provenance:</strong> Kodansha (1987) / Vintage International (Jay Rubin Translation) • 3,630 Paragraphs (11 Chapters) • 10.5 hrs saved
    </div>
    <div class="golden-test-banner">
      <strong>The Standard of Total Replacement:</strong> Engineered to eliminate the need to read the source text. Every single chapter is forensic, capturing the acoustic trauma, the phantom well, the Werther effect, the strawberry shortcake heuristic, and the final phone booth void.
    </div>
  </header>
...
const outHtmlPath = path.join(__dirname, '..', 'docs', 'distillations', 'norwegian-wood', 'index.html');
