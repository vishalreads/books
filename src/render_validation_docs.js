...
const targetDir = path.join(__dirname, '..', 'docs', 'distillations', 'bhagat-singh-a-life-in-revolution', 'validation');
const questionsJsonPath = path.join(targetDir, 'questions.json');
const questions = JSON.parse(fs.readFileSync(questionsJsonPath, 'utf8'));

// Generate QUESTIONS.md
let mdContent = `# BKRS Source-Derived Forensic Validation Set
## 50 Independent Verification Archetypes instantiated for *Bhagat Singh: A Life in Revolution* (Juss, 2022)

**Document:** QUESTIONS.md  
**Benchmark Target:** *Bhagat Singh: A Life in Revolution* (Satvinder S. Juss, Penguin Viking, 2022)  
**Milestone:** Milestone 3 — Step 2 Gateway Certification  
**Status:** **FROZEN & CERTIFIED**  
**Total Questions:** 50  
**Answer Key Separation:** The answer key is maintained separately in an inaccessible audit module; these questions contain zero answers or leading hints.

---

## Forensic Dimension Allocation Matrix

| Dimension | Question IDs | Count | Focus & Evaluation Standard |
|---|:---:|:---:|---|
| **D1: Macro-Historical Structure & Chronology** | VAL-HIST-D1-01 to D1-07 | 7 | Exact calendar anchors, temporal pacing, institutional progressions |
| **D2: Primary Text & Verbatim Subject Utterance** | VAL-HIST-D2-08 to D2-15 | 8 | Verbatim declarations, manifesto quotes, ideological definitions |
| **D3: Micro-Texture & Evidentiary Forensics** | VAL-HIST-D3-16 to D3-23 | 8 | Somatic incarceration realism, ballistics, disguises, spatial scenes |
| **D4: Multi-Step Causal Historical Mechanics** | VAL-HIST-D4-24 to D4-31 | 8 | Anti-post-hoc causal chains, operational errors, legal escalations |
| **D5: Epistemic Demarcation: Author vs Subject** | VAL-HIST-D5-32 to D5-39 | 8 | Strict isolation of biographer conjecture and approver coercion |
| **D6: Historiographical Disputes & Conflicts** | VAL-HIST-D6-40 to D6-45 | 6 | Competing accounts of Gandhi-Irwin talks, covert disposal, ballistics |
| **D7: Cross-Event Dependencies & Trajectories** | VAL-HIST-D7-46 to D7-50 | 5 | Long-term interpersonal relations, generational rupture, myth vs reality |
| **TOTAL** | — | **50** | **Complete coverage of all 50 abstract protocol archetypes** |
...
questions.forEach((q, idx) => {
  mdContent += `### [${q.question_id}] ${q.archetype}\n`;
  mdContent += `- **Dimension:** ${q.dimension}\n`;
  mdContent += `- **Question:** ${q.question}\n`;
  mdContent += `- **Expected Information Type:** \`${q.expected_information_type}\`\n`;
  mdContent += `- **Source Provenance:** \`${q.source_provenance.chapter}\` (\`${q.source_provenance.source_locator}\`)\n`;
  mdContent += `- **Unit IDs:** \`${q.source_provenance.unit_ids.join(', ')}\`\n`;
  mdContent += `- **Difficulty:** \`${q.difficulty}\` | **Materiality:** \`${q.materiality}\`\n\n`;
});

const questionsMdPath = path.join(targetDir, 'QUESTIONS.md');
fs.writeFileSync(questionsMdPath, mdContent, 'utf8');
console.log(`Saved QUESTIONS.md to ${questionsMdPath}`);

// Generate QUESTION_AUDIT.md
let auditMd = `# BKRS Validation Set Independent Integrity & Answerability Audit
...
const auditMdPath = path.join(targetDir, 'QUESTION_AUDIT.md');
fs.writeFileSync(auditMdPath, auditMd, 'utf8');
console.log(`Saved QUESTION_AUDIT.md to ${auditMdPath}`);
